...
describe('workspace browser rows', () => {
  ...
  it('idle hover card shows the Idle status line', () => {
    ...
    try {
      ...
      fireEvent.pointerEnter(screen.getByRole('treeitem').parentElement as HTMLElement)
      act(() => { vi.advanceTimersByTime(800) })
      // Row label (visually hidden) plus the hover card's status line, each beside an idle dot.
      const idle = screen.getAllByText('空闲')
      expect(idle).toHaveLength(2)
      for (const label of idle) expect(label.parentElement?.querySelector('[data-state="idle"]')).not.toBeNull()
